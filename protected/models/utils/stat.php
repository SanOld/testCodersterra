...
include_once(Yii::app()->getBasePath().'/extensions/GEO/geoip.php'); 

$delta = 60*60;
$table = 'spi_visit';
$agentData = array();
$agentData = parse_user_agent();

$os = $agentData['platform'];
$browser = $agentData['browser'];
$browserVersion = $agentData['version'];
...
$hint = $_SERVER['REQUEST_URI'];
$cookie = isset($_COOKIE[ __count_date ]) ? (int) $_COOKIE[ __count_date ] : setcookie(__count_date, time(), time() + 60 * 60 * 24);


$data = array(
    'browser' => array('name' => $browser, 'id' => '')
  , 'os'      => array('name' => $os, 'id' => '')
  , 'city'    => array('name' => $city, 'id' => '')
  , 'ref'     => array('name' => $referer, 'id' => '')
  , 'hint'    => array('name' => $hint, 'id' => '')
  , 'ip'      => array('name' => $visitor_ip, 'id' => '')
  , 'cookie'  => array('name' => $cookie, 'id' => '')
);

$visit = array();
$i = 0;
foreach ( $data as $key => $value ) {
  if (isset($_SESSION[$key]) && $_SESSION[$key] == $value['name'] ){
    $i++;
  } 
  $id = Yii::app ()->db->createCommand()->select('id')
    ->from($key)
    ->where('name = :name', array(':name' => $value['name']))
    ->queryScalar();

  if(!$id){
    Yii::app()->db->createCommand()->insert($key, array('name' => $value['name']));
...
//    unset($_SESSION[$key])  ;
  ...
  $visit[$key.'_id'] = $data[$key]['id'];
}

if (count($visit) == $i ) {
  $newtime = time() - $delta;
   $res = Yii::app ()->db->createCommand()->update($table, array(
                                                                  'count' => new CDbExpression('count + 1'))
                                                                , '(id='.$_SESSION['visit_id'].' AND time >'.$newtime.')'
                                                                 );
   ...
   if (!$res){
      $visit['count'] = 1;
      $visit['time'] = time();
      Yii::app ()->db->createCommand()->insert($table, $visit);
      $_SESSION['visit_id'] = Yii::app()->db->getLastInsertID();
   }
